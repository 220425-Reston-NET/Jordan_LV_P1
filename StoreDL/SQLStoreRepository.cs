  ...
  private List<StoreAbility> GiveAbilityStoreItem(int p_storeItemId)
        {
         ...
         string SQLquery = @"select p.StoreBrand, pa.Color, pa.Material, pa.Price from Shoe
                        inner join store_abilities pa on p.ItemId = pa.ItemId
                        inner join Ability a on a.abId = pa.abId
                        where p.StoreItemId = @ItemId";
